- finish changing shaders to new system of getting the shader from the asset manager
- change releasing assets using AssetManager.ResleaseAssetHandle to take in the handle instead of just the asset_id that way the asset manager can invalidate the handle automatically and not have some weird dangling asset id reference 
