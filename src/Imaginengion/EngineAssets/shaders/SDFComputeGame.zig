...
const LoadTexturePixel = SDFShared.LoadTexturePixel;
const SampleSampler = SDFShared.SampleSampler;
const StoreTexturePixel = SDFShared.StoreTexturePixel;
...
export fn main() callconv(.{ .spirv_kernel = .{ .x = 8, .y = 8, .z = 1 } }) void {
    ...
    const pixels = @Vector(2, i32){ @as(i32, @intCast(global[0])), @as(i32, @intCast(global[1])) };

    const sample = LoadTexturePixel(.{ .descriptor = .{ .set = 2, .binding = 1 } }, pixels);
    ...
    marcher.March(QuadsSSBO.ptr, GlyphsSSBO.ptr, CameraUBO.mPerspectiveFar, SampleSampler);

    //traverse ray tree backwards to obtain final output color
    const march_color = marcher.GenerateColor(ShadingSSBO.ptr, TexturesArray, SampleSampler);
    ...
    StoreTexturePixel(.{ .descriptor = .{ .set = 2, .binding = 1 } }, pixels, final_color.ToVector());
}
