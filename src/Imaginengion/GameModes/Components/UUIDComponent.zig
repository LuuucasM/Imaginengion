...
pub fn jsonParse(frame_allocator: std.mem.Allocator, reader: anytype, options: std.json.ParseOptions) std.json.ParseError(@TypeOf(reader.*))!UUIDComponent {
    ...
    while (true) {
        ...
        if (std.mem.eql(u8, field_name, "UUID")) {
            const game_mode_uuid = try std.json.innerParse(u64, frame_allocator, reader, options);
            std.debug.assert(engine_context.mSerializer.mCurrDeserialize.requester == .GameMode);
            const game_mode = engine_context.mSerializer.mCurrDeserialize.requester.GameMode;
            game_mode.mSceneManager.AddUUID(engine_context.EngineAllocator(), game_mode_uuid, game_mode.mEntityID) catch @panic("this failed");
            result.ID = game_mode_uuid;
        }
    }
