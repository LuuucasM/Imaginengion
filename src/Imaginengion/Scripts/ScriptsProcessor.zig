...
const KeyPressedEvent = SystemEvent.KeyPressedEvent;
...
pub fn OnKeyPressedEvent(scene_manager: *SceneManager, e: KeyPressedEvent) !bool {
    ...
    const group = try scene_manager.mECSManager.GetGroup(.{ .Component = OnKeyPressedScript }, allocator);
    ...
    while (iter.next()) |*scene_layer| {
        ...
        for (group.items) |script_id| {
            ...
            const run_func = script_asset.mLib.lookup(*const fn (*EngineContext, *const std.mem.Allocator, *Entity, *const KeyPressedEvent) callconv(.C) bool, "Run").?;
