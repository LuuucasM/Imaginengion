    ...
    return struct {
        ...
        pub fn BindColorAttachment(self: Self, render_pass: *anyopaque, attachment_index: usize, slot: u32) void {
            ...
            const binding = sdl.SDL_GPUTextureSamplerBinding{
                .texture = self.mColorTextures[attachment_index],
                .sampler = self.mColorSamplers[attachment_index],
            };
        ...
        fn Create(self: *Self, engine_context: *EngineContext) void {
            ...
            inline for (color_texture_formats, 0..) |format, i| {
                ...
                std.debug.assert(self.mColorTextures[i] != null);
                ...
                self.mColorSamplers[i] = sdl.SDL_CreateGPUSampler(device, &sampler_info);
                std.debug.assert(self.mColorSamplers[i] != null);
            }
            if (HasDepth) {
        ...
        fn Destroy(self: *Self, engine_context: *EngineContext) void {
            const device: *sdl.SDL_GPUDevice = @ptrCast(@alignCast(engine_context.mRenderer.mRenderContext.GetDevice()));
            inline for (0..color_texture_formats.len) |i| {
                if (self.mColorTextures[i]) |texture| sdl.SDL_ReleaseGPUTexture(device, texture);
                if (self.mColorSamplers[i]) |sampler| sdl.SDL_ReleaseGPUSampler(device, sampler);
                self.mColorTextures[i] = null;
                self.mColorSamplers[i] = null;
            }
            if (self.mDepthTexture) |t| sdl.SDL_ReleaseGPUTexture(self.mDevice, t);
