...
pub fn OnManagerEvents(self: *PManager, engine_context: *EngineContext, event: EventData.EventT) anyerror!EventResult {
    switch (event) {
        .DestroyPlayer => |e| try self.mECSManager.DestroyEntity(engine_context, e.Player.mID),
        .Default => unreachable,
    }
    return .Continue;
}
